...
export function registerNuxtSingleCommand(program: Command) {
  program
    .command("nuxt:single <type> <name>")
    .description(
    ...
    .action(async () => {
      ...
      cmd.options.forEach((option: string) => {
        if (option === "Account/Anthentication") {
          // NitroCreateAccountModule(frameworkPath);
          // createAuthNitro(frameworkPath);
        }
        ...
        if (option === "ALL") {
          ...
          if (Array.isArray(entitiesJsonFile)) {
            entitiesJsonFile.forEach((entity: IEntityJson) => {
              ...
              // createModuleEntityNitro(frameworkPath, entity);
            });
          }
          // createSeederNitro(frameworkPath, entitiesJsonFile);
