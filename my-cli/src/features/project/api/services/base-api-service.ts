...
class BaseService {
  ...
  constructor(token: string) {
    ...
    logInfo(`BaseService initialized with token: ${token}`);
  }
}
// La classe de service API générique qui implémente l'interface.
export class BaseApiService<T, TCreateDto, TUpdateDto>
  implements IApiService<T, TCreateDto, TUpdateDto>
{
  ...
  getAll(): ApiResponse<T[]> {
    logInfo(`Fetching all resources from ${this.baseUrlApi}`);
    // Une véritable implémentation utiliserait 'fetch' ici.
    return Promise.resolve([]);
  }

  getById(id: number): ApiResponse<T> {
    logInfo(`Fetching resource with id ${id} from ${this.baseUrlApi}`);
    return Promise.resolve(null);
  }

  create(body: TCreateDto): ApiResponse<T> {
    logInfo(`Creating resource at ${this.baseUrlApi} with body: ${body}`);
    return Promise.resolve(null);
  }

  update(id: number, body: TUpdateDto): ApiResponse<T> {
    logInfo(`Updating resource with id ${id} at ${this.baseUrlApi}`);
    return Promise.resolve(null);
  }

  delete(id: number): ApiResponse<boolean> {
    logInfo(`Deleting resource with id ${id} at ${this.baseUrlApi}`);
...
> {
  ...
  constructor(baseUrl: string) {
    ...
    logInfo(`UserService endpoint is:", ${this.baseUrlApi}`);
  }
}
