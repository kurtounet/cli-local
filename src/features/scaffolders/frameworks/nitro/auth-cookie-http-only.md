...
export default defineEventHandler(async (event) => {
  ...
  const [user] = await db
    .select()
    .from(users)
    .where(eq(users.id, Number(jwt.sub)))
    .limit(1);
  return {
    success: true,
    data: user ? { id: user.id, email: user.email, name: user.name } : null,
  };
});
```
...
export default defineEventHandler((event) => {
  ...
  if (MUTATING.has(method)) {
    ...
    if (!headerToken || headerToken !== cookieToken) {
      throw createError({
        statusCode: 403,
        statusMessage: "CSRF token invalid or missing",
      });
    }
  }
});
...
export default defineEventHandler((event) => {
  ...
  if (MUTATING.has(method)) {
    ...
    if (!isAllowedByOriginOrReferer(event)) {
      throw createError({
        statusCode: 403,
        statusMessage: "Origin/Referer non autorise",
      });
    }
    ...
    if (!headerToken || headerToken !== cookieToken) {
      throw createError({
        statusCode: 403,
        statusMessage: "CSRF token invalide ou manquant",
      });
    }
  }
});
...
function hashToken(t: string) {
  return createHash("sha256").update(t).digest("base64");
}
...
const row = await db.query.authRefreshToken.findFirst({
  where: eq(t.jti, payload.jti),
});
...
const WINDOW_MS = 60_000,
  LIMIT = 10; // 10 req / min / IP sur endpoints sensibles
